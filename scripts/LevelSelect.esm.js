...
class LevelSelect extends Common{
   ...
   loadLevel(level) {
      if (media.backgroundImage && media.diamondSprite && media.backgroundMusic && media.swapSound) {
         game.playLevel(level);
         return;
      };

      if (!media.diamondSprite) {
         media.diamondSprite = loader.loadImage('images/diamonds-transparent.png');
      };
