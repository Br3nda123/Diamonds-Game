...
class Game extends Common {
   ...
   playLevel(level) {
      ...
      this.gameState = new GameState(level, numberOfMovements, pointsToWin ,board, media.diamondSprite);
   ...
   handleMouseClick() {
      ...
      if (mouseController.state === 1) {
         ...
         console.log(mouseController.firstClick.x)
         console.log(mouseController.firstClick.y)
         console.log(mouseController.firstClick)
         console.log(mouseController.secondClick)
         console.log(Math.abs(mouseController.firstClick.x - mouseController.secondClick.x) + 
         ...
         this.gameState.decreasePoitsMovement();
         mouseController.state = 0;
      };
   ...
   findMatches() {
      this.gameState.getGameBoard().forEach((diamond, index, diamonds) => {
         if (diamond.kind === EMPTY_BLOCK || index < DIAMONDS_ARRAY_WIDTH || index === LAST_ELEMENT_DIAMOND_ARRAY) {
            return;
         };
   ...
   moveDiamonds() {
      ...
      this.gameState.getGameBoard().forEach(diamond => {
         ...
         for (let speedSwap = 0; speedSwap < SWAPING_SPPED; speedSwap++) {
   ...
   countScores() {
      ...
      if (!this.gameState.getIsMoving() && this.scores) {
         this.gameState.incresePlayerPoints(this.scores);
      };
   };
   ...
   revertSwap() {
      if (this.gameState.getIsSwaping() && !this.gameState.getIsMoving()) {
         if (!this.scores) {
            this.swapDiamonds();
            this.gameState.increasePoitsMovement();
         };
   ...
   checkPosibilityMovement() {
      ...
      this.isPossibleToMove = this.gameState.getGameBoard().some((diamond, index, diamonds) => {
         ...
         if (
            ...
            && diamond.kind === diamonds[index + 1 - DIAMONDS_ARRAY_WIDTH].kind
            && diamond.kind === diamonds[index + 1 + DIAMONDS_ARRAY_WIDTH].kind
         ...
         if (
            ...
            && diamond.kind === diamonds[index + 1 + DIAMONDS_ARRAY_WIDTH].kind
            && diamond.kind === diamonds[index + 1 + DIAMONDS_ARRAY_WIDTH * 2].kind
         ) {
            return true;
         };
         ...
         // move down => check if is in the column
         if (
         ...
         // move up => check if is in the column
         if (
   ...
   checkEndOfGame() {
      if (!this.gameState.getLeftMovement() && !this.gameState.getIsMoving() && !this.gameState.getIsSwaping()) {
         ...
         this.animationFram = window.requestAnimationFrame(() => this.animate());
      }
   };
